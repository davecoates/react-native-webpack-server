...
function Server(options) {
  ...
  this.webpackBaseURL = 'http://' + this.hostname + ':' + this.port;
  ...
  this._attachWebpackDevServerMiddleware()
    .then(function() {
      this.server.use(this.handleRequest.bind(this));
      this._startPackageServer.bind(this)()
    }.bind(this));
}
...
Server.prototype = {
  ...
  handleIndexRequest: function(req, res, next) {
    Promise.props({
  ...
  _attachWebpackDevServerMiddleware: function() {
    var webpackConfig = this.webpackConfig;
    var webpackURL = 'http://' + this.hostname + ':' + this.port;
    var hot = this.hot;
    var publicPath = hot ? (webpackURL + '/index.ios.js') : null;
    return getReactNativeExternals().then(function(reactNativeExternals) {
      return new Promise(function(resolve, reject) {
        ...
        this.server.use(require('webpack-dev-middleware')(compiler, {
          noInfo: true,
